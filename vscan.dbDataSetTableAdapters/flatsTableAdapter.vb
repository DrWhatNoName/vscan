...
Namespace vscan.dbDataSetTableAdapters
	...
	Public Class flatsTableAdapter
		Inherits Component
		...
		Private Shared Sub __ENCAddToList(value As Object)
			Dim _ENCList As List(Of WeakReference) = flatsTableAdapter.__ENCList
			Dim flag As Boolean = False
			' The following expression was wrapped in a checked-statement
			Try
				Monitor.Enter(_ENCList, flag)
				Dim flag2 As Boolean = flatsTableAdapter.__ENCList.Count = flatsTableAdapter.__ENCList.Capacity
				If flag2 Then
					Dim num As Integer = 0
					Dim arg_44_0 As Integer = 0
					Dim num2 As Integer = flatsTableAdapter.__ENCList.Count - 1
					...
					While True
						...
						If arg_95_0 > num4 Then
							Exit While
						End If
						Dim weakReference As WeakReference = flatsTableAdapter.__ENCList(num3)
						flag2 = weakReference.IsAlive
						If flag2 Then
							Dim flag3 As Boolean = num3 <> num
							If flag3 Then
								flatsTableAdapter.__ENCList(num) = flatsTableAdapter.__ENCList(num3)
							End If
							num += 1
						End If
						num3 += 1
					End While
					flatsTableAdapter.__ENCList.RemoveRange(num, flatsTableAdapter.__ENCList.Count - num)
					flatsTableAdapter.__ENCList.Capacity = flatsTableAdapter.__ENCList.Count
				End If
				flatsTableAdapter.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
			Finally
		...
		<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), DebuggerNonUserCode()>
		Public Sub New()
			flatsTableAdapter.__ENCAddToList(Me)
		...
		Private Sub InitAdapter()
			...
			Me._adapter.DeleteCommand.CommandText = "DELETE FROM `flats` WHERE (((? = 1 AND `x` IS NULL) OR (`x` = ?)) AND ((? = 1 AND `y` IS NULL) OR (`y` = ?)) AND ((? = 1 AND `level` IS NULL) OR (`level` = ?)) AND (`ID` = ?))"
			...
			Me._adapter.InsertCommand.CommandText = "INSERT INTO `flats` (`x`, `y`, `level`) VALUES (?, ?, ?)"
			...
			Me._adapter.UpdateCommand.CommandText = "UPDATE `flats` SET `x` = ?, `y` = ?, `level` = ? WHERE (((? = 1 AND `x` IS NULL) OR (`x` = ?)) AND ((? = 1 AND `y` IS NULL) OR (`y` = ?)) AND ((? = 1 AND `level` IS NULL) OR (`level` = ?)) AND (`ID` = ?))"
		...
		Private Sub InitCommandCollection()
			...
			Me._commandCollection(0).CommandText = "SELECT x, y, [level], ID FROM flats"
		...
		Public Overridable Function Fill(dataTable As dbDataSet.flatsDataTable) As Integer
		...
		Public Overridable Function GetData() As dbDataSet.flatsDataTable
			Me.Adapter.SelectCommand = Me.CommandCollection(0)
			Dim dataTable As dbDataSet.flatsDataTable = New dbDataSet.flatsDataTable()
			Me.Adapter.Fill(dataTable)
			Return dataTable
		End Function

		<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), HelpKeyword("vs.data.TableAdapter"), DebuggerNonUserCode()>
		Public Overridable Function Update(dataTable As dbDataSet.flatsDataTable) As Integer
			Return Me.Adapter.Update(dataTable)
		End Function

		<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), HelpKeyword("vs.data.TableAdapter"), DebuggerNonUserCode()>
		Public Overridable Function Update(dataSet As dbDataSet) As Integer
			Return Me.Adapter.Update(dataSet, "flats")
		End Function
		...
		Public Overridable Function Delete(Original_x As Integer?, Original_y As Integer?, Original_level As Integer?, Original_ID As Integer) As Integer
		...
		Public Overridable Function Insert(x As Integer?, y As Integer?, level As Integer?) As Integer
		...
		Public Overridable Function Update(x As Integer?, y As Integer?, level As Integer?, Original_x As Integer?, Original_y As Integer?, Original_level As Integer?, Original_ID As Integer) As Integer
			Dim flag As Boolean = x.HasValue
			If flag Then
			...
			If flag Then
				...
				Me.Adapter.UpdateCommand.Parameters(8).Value = Original_level.Value
			Else
