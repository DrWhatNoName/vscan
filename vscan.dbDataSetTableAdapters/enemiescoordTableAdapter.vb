...
Namespace vscan.dbDataSetTableAdapters
	...
	Public Class enemiescoordTableAdapter
		Inherits Component
		...
		Private Shared Sub __ENCAddToList(value As Object)
			Dim _ENCList As List(Of WeakReference) = enemiescoordTableAdapter.__ENCList
			Dim flag As Boolean = False
			' The following expression was wrapped in a checked-statement
			Try
				Monitor.Enter(_ENCList, flag)
				Dim flag2 As Boolean = enemiescoordTableAdapter.__ENCList.Count = enemiescoordTableAdapter.__ENCList.Capacity
				If flag2 Then
					Dim num As Integer = 0
					Dim arg_44_0 As Integer = 0
					Dim num2 As Integer = enemiescoordTableAdapter.__ENCList.Count - 1
					...
					While True
						...
						If arg_95_0 > num4 Then
							Exit While
						End If
						Dim weakReference As WeakReference = enemiescoordTableAdapter.__ENCList(num3)
						flag2 = weakReference.IsAlive
						If flag2 Then
							Dim flag3 As Boolean = num3 <> num
							If flag3 Then
								enemiescoordTableAdapter.__ENCList(num) = enemiescoordTableAdapter.__ENCList(num3)
							End If
							num += 1
						End If
						num3 += 1
					End While
					enemiescoordTableAdapter.__ENCList.RemoveRange(num, enemiescoordTableAdapter.__ENCList.Count - num)
					enemiescoordTableAdapter.__ENCList.Capacity = enemiescoordTableAdapter.__ENCList.Count
				End If
				enemiescoordTableAdapter.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
			Finally
		...
		<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), DebuggerNonUserCode()>
		Public Sub New()
			enemiescoordTableAdapter.__ENCAddToList(Me)
		...
		Private Sub InitAdapter()
			...
			tableMapping.DataSetTable = "enemiescoord"
		...
		Private Sub InitCommandCollection()
			...
			Me._commandCollection(0).CommandText = "SELECT id, alliance, player, x, y, citie FROM enemiescoord"
		...
		Public Overridable Function Fill(dataTable As dbDataSet.enemiescoordDataTable) As Integer
		...
		Public Overridable Function GetData() As dbDataSet.enemiescoordDataTable
			Me.Adapter.SelectCommand = Me.CommandCollection(0)
			Dim dataTable As dbDataSet.enemiescoordDataTable = New dbDataSet.enemiescoordDataTable()
