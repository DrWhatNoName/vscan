...
Namespace vscan.dbDataSetTableAdapters
	...
	Public Class npcTableAdapter
		...
		Private Shared Sub __ENCAddToList(value As Object)
			Dim _ENCList As List(Of WeakReference) = npcTableAdapter.__ENCList
			Dim flag As Boolean = False
			' The following expression was wrapped in a checked-statement
			Try
				Monitor.Enter(_ENCList, flag)
				Dim flag2 As Boolean = npcTableAdapter.__ENCList.Count = npcTableAdapter.__ENCList.Capacity
				If flag2 Then
					Dim num As Integer = 0
					Dim arg_44_0 As Integer = 0
					Dim num2 As Integer = npcTableAdapter.__ENCList.Count - 1
					...
					While True
						...
						If arg_95_0 > num4 Then
							Exit While
						End If
						Dim weakReference As WeakReference = npcTableAdapter.__ENCList(num3)
						flag2 = weakReference.IsAlive
						If flag2 Then
							Dim flag3 As Boolean = num3 <> num
							If flag3 Then
								npcTableAdapter.__ENCList(num) = npcTableAdapter.__ENCList(num3)
							End If
							num += 1
						End If
						num3 += 1
					End While
					npcTableAdapter.__ENCList.RemoveRange(num, npcTableAdapter.__ENCList.Count - num)
					npcTableAdapter.__ENCList.Capacity = npcTableAdapter.__ENCList.Count
				End If
				npcTableAdapter.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
			Finally
		...
		<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), DebuggerNonUserCode()>
		Public Sub New()
			npcTableAdapter.__ENCAddToList(Me)
		...
		Private Sub InitAdapter()
			...
			Me._adapter.DeleteCommand.CommandText = "DELETE FROM `npc` WHERE (((? = 1 AND `x` IS NULL) OR (`x` = ?)) AND ((? = 1 AND `y` IS NULL) OR (`y` = ?)) AND ((? = 1 AND `level` IS NULL) OR (`level` = ?)) AND (`ID` = ?))"
			...
			Me._adapter.InsertCommand.CommandText = "INSERT INTO `npc` (`x`, `y`, `level`) VALUES (?, ?, ?)"
			...
			Me._adapter.UpdateCommand.CommandText = "UPDATE `npc` SET `x` = ?, `y` = ?, `level` = ? WHERE (((? = 1 AND `x` IS NULL) OR (`x` = ?)) AND ((? = 1 AND `y` IS NULL) OR (`y` = ?)) AND ((? = 1 AND `level` IS NULL) OR (`level` = ?)) AND (`ID` = ?))"
		...
		Private Sub InitCommandCollection()
			...
			Me._commandCollection(0).CommandText = "SELECT x, y, [level], ID FROM npc"
		...
		Public Overridable Function Fill(dataTable As dbDataSet.npcDataTable) As Integer
		...
		Public Overridable Function GetData() As dbDataSet.npcDataTable
			Me.Adapter.SelectCommand = Me.CommandCollection(0)
			Dim dataTable As dbDataSet.npcDataTable = New dbDataSet.npcDataTable()
			Me.Adapter.Fill(dataTable)
			Return dataTable
		End Function

		<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), HelpKeyword("vs.data.TableAdapter"), DebuggerNonUserCode()>
		Public Overridable Function Update(dataTable As dbDataSet.npcDataTable) As Integer
