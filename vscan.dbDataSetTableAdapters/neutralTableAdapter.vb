...
Namespace vscan.dbDataSetTableAdapters
	...
	Public Class neutralTableAdapter
		Inherits Component
		...
		Private Shared Sub __ENCAddToList(value As Object)
			Dim _ENCList As List(Of WeakReference) = neutralTableAdapter.__ENCList
			Dim flag As Boolean = False
			' The following expression was wrapped in a checked-statement
			Try
				Monitor.Enter(_ENCList, flag)
				Dim flag2 As Boolean = neutralTableAdapter.__ENCList.Count = neutralTableAdapter.__ENCList.Capacity
				If flag2 Then
					Dim num As Integer = 0
					Dim arg_44_0 As Integer = 0
					Dim num2 As Integer = neutralTableAdapter.__ENCList.Count - 1
					...
					While True
						...
						If arg_95_0 > num4 Then
							Exit While
						End If
						Dim weakReference As WeakReference = neutralTableAdapter.__ENCList(num3)
						flag2 = weakReference.IsAlive
						If flag2 Then
							Dim flag3 As Boolean = num3 <> num
							If flag3 Then
								neutralTableAdapter.__ENCList(num) = neutralTableAdapter.__ENCList(num3)
							End If
							num += 1
						End If
						num3 += 1
					End While
					neutralTableAdapter.__ENCList.RemoveRange(num, neutralTableAdapter.__ENCList.Count - num)
					neutralTableAdapter.__ENCList.Capacity = neutralTableAdapter.__ENCList.Count
				End If
				neutralTableAdapter.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
			Finally
		...
		<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), DebuggerNonUserCode()>
		Public Sub New()
			neutralTableAdapter.__ENCAddToList(Me)
		...
		Private Sub InitAdapter()
			...
			tableMapping.DataSetTable = "neutral"
			...
			Me._adapter.DeleteCommand.CommandText = "DELETE FROM `neutral` WHERE ((`id` = ?) AND ((? = 1 AND `alliance` IS NULL) OR (`alliance` = ?)) AND ((? = 1 AND `player` IS NULL) OR (`player` = ?)) AND ((? = 1 AND `x` IS NULL) OR (`x` = ?)) AND ((? = 1 AND `y` IS NULL) OR (`y` = ?)))"
			...
			Me._adapter.InsertCommand.CommandText = "INSERT INTO `neutral` (`alliance`, `player`, `x`, `y`) VALUES (?, ?, ?, ?)"
			...
			Me._adapter.UpdateCommand.CommandText = "UPDATE `neutral` SET `alliance` = ?, `player` = ?, `x` = ?, `y` = ? WHERE ((`id` = ?) AND ((? = 1 AND `alliance` IS NULL) OR (`alliance` = ?)) AND ((? = 1 AND `player` IS NULL) OR (`player` = ?)) AND ((? = 1 AND `x` IS NULL) OR (`x` = ?)) AND ((? = 1 AND `y` IS NULL) OR (`y` = ?)))"
		...
		Private Sub InitCommandCollection()
			...
			Me._commandCollection(0).CommandText = "SELECT id, alliance, player, x, y FROM neutral"
		...
		Public Overridable Function Fill(dataTable As dbDataSet.neutralDataTable) As Integer
		...
		Public Overridable Function GetData() As dbDataSet.neutralDataTable
			Me.Adapter.SelectCommand = Me.CommandCollection(0)
			Dim dataTable As dbDataSet.neutralDataTable = New dbDataSet.neutralDataTable()
			Me.Adapter.Fill(dataTable)
			Return dataTable
		End Function

		<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), HelpKeyword("vs.data.TableAdapter"), DebuggerNonUserCode()>
		Public Overridable Function Update(dataTable As dbDataSet.neutralDataTable) As Integer
			Return Me.Adapter.Update(dataTable)
		End Function

		<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), HelpKeyword("vs.data.TableAdapter"), DebuggerNonUserCode()>
		Public Overridable Function Update(dataSet As dbDataSet) As Integer
			Return Me.Adapter.Update(dataSet, "neutral")
		End Function
