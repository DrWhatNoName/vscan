...
Namespace vscan.dbDataSetTableAdapters
	...
	Public Class enemiesTableAdapter
		Inherits Component
		...
		Private Shared Sub __ENCAddToList(value As Object)
			Dim _ENCList As List(Of WeakReference) = enemiesTableAdapter.__ENCList
			Dim flag As Boolean = False
			' The following expression was wrapped in a checked-statement
			Try
				Monitor.Enter(_ENCList, flag)
				Dim flag2 As Boolean = enemiesTableAdapter.__ENCList.Count = enemiesTableAdapter.__ENCList.Capacity
				If flag2 Then
					Dim num As Integer = 0
					Dim arg_44_0 As Integer = 0
					Dim num2 As Integer = enemiesTableAdapter.__ENCList.Count - 1
					...
					While True
						...
						If arg_95_0 > num4 Then
							Exit While
						End If
						Dim weakReference As WeakReference = enemiesTableAdapter.__ENCList(num3)
						flag2 = weakReference.IsAlive
						If flag2 Then
							Dim flag3 As Boolean = num3 <> num
							If flag3 Then
								enemiesTableAdapter.__ENCList(num) = enemiesTableAdapter.__ENCList(num3)
							End If
							num += 1
						End If
						num3 += 1
					End While
					enemiesTableAdapter.__ENCList.RemoveRange(num, enemiesTableAdapter.__ENCList.Count - num)
					enemiesTableAdapter.__ENCList.Capacity = enemiesTableAdapter.__ENCList.Count
				End If
				enemiesTableAdapter.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
			Finally
		...
		<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), DebuggerNonUserCode()>
		Public Sub New()
			enemiesTableAdapter.__ENCAddToList(Me)
		...
		Private Sub InitAdapter()
			...
			Me._adapter.DeleteCommand.CommandText = "DELETE FROM `enemies` WHERE ((`id` = ?) AND ((? = 1 AND `alliance` IS NULL) OR (`alliance` = ?)) AND ((? = 1 AND `player` IS NULL) OR (`player` = ?)) AND ((? = 1 AND `x` IS NULL) OR (`x` = ?)) AND ((? = 1 AND `y` IS NULL) OR (`y` = ?)))"
			...
			Me._adapter.InsertCommand.CommandText = "INSERT INTO `enemies` (`alliance`, `player`, `x`, `y`) VALUES (?, ?, ?, ?)"
			...
			Me._adapter.UpdateCommand.CommandText = "UPDATE `enemies` SET `alliance` = ?, `player` = ?, `x` = ?, `y` = ? WHERE ((`id` = ?) AND ((? = 1 AND `alliance` IS NULL) OR (`alliance` = ?)) AND ((? = 1 AND `player` IS NULL) OR (`player` = ?)) AND ((? = 1 AND `x` IS NULL) OR (`x` = ?)) AND ((? = 1 AND `y` IS NULL) OR (`y` = ?)))"
		...
		Private Sub InitCommandCollection()
			...
			Me._commandCollection(0).CommandText = "SELECT id, alliance, player, x, y FROM enemies"
		...
		Public Overridable Function Fill(dataTable As dbDataSet.enemiesDataTable) As Integer
		...
		Public Overridable Function GetData() As dbDataSet.enemiesDataTable
			Me.Adapter.SelectCommand = Me.CommandCollection(0)
			Dim dataTable As dbDataSet.enemiesDataTable = New dbDataSet.enemiesDataTable()
			Me.Adapter.Fill(dataTable)
			Return dataTable
		End Function

		<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), HelpKeyword("vs.data.TableAdapter"), DebuggerNonUserCode()>
		Public Overridable Function Update(dataTable As dbDataSet.enemiesDataTable) As Integer
			Return Me.Adapter.Update(dataTable)
		End Function

		<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), HelpKeyword("vs.data.TableAdapter"), DebuggerNonUserCode()>
		Public Overridable Function Update(dataSet As dbDataSet) As Integer
			Return Me.Adapter.Update(dataSet, "enemies")
		End Function
