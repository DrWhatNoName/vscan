...
Namespace vscan.dbDataSetTableAdapters
	...
	Public Class altTableAdapter
		...
		Private Shared Sub __ENCAddToList(value As Object)
			Dim _ENCList As List(Of WeakReference) = altTableAdapter.__ENCList
			Dim flag As Boolean = False
			' The following expression was wrapped in a checked-statement
			Try
				Monitor.Enter(_ENCList, flag)
				Dim flag2 As Boolean = altTableAdapter.__ENCList.Count = altTableAdapter.__ENCList.Capacity
				If flag2 Then
					Dim num As Integer = 0
					Dim arg_44_0 As Integer = 0
					Dim num2 As Integer = altTableAdapter.__ENCList.Count - 1
					...
					While True
						...
						If arg_95_0 > num4 Then
							Exit While
						End If
						Dim weakReference As WeakReference = altTableAdapter.__ENCList(num3)
						flag2 = weakReference.IsAlive
						If flag2 Then
							Dim flag3 As Boolean = num3 <> num
							If flag3 Then
								altTableAdapter.__ENCList(num) = altTableAdapter.__ENCList(num3)
							End If
							num += 1
						End If
						num3 += 1
					End While
					altTableAdapter.__ENCList.RemoveRange(num, altTableAdapter.__ENCList.Count - num)
					altTableAdapter.__ENCList.Capacity = altTableAdapter.__ENCList.Count
				End If
				altTableAdapter.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
			Finally
		...
		<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), DebuggerNonUserCode()>
		Public Sub New()
			altTableAdapter.__ENCAddToList(Me)
		...
		Private Sub InitAdapter()
			...
			Me._adapter.DeleteCommand.CommandText = "DELETE FROM `alt` WHERE ((`id` = ?) AND ((? = 1 AND `alliance` IS NULL) OR (`alliance` = ?)) AND ((? = 1 AND `player` IS NULL) OR (`player` = ?)) AND ((? = 1 AND `x` IS NULL) OR (`x` = ?)) AND ((? = 1 AND `y` IS NULL) OR (`y` = ?)))"
			...
			Me._adapter.InsertCommand.CommandText = "INSERT INTO `alt` (`alliance`, `player`, `x`, `y`) VALUES (?, ?, ?, ?)"
			...
			Me._adapter.UpdateCommand.CommandText = "UPDATE `alt` SET `alliance` = ?, `player` = ?, `x` = ?, `y` = ? WHERE ((`id` = ?) AND ((? = 1 AND `alliance` IS NULL) OR (`alliance` = ?)) AND ((? = 1 AND `player` IS NULL) OR (`player` = ?)) AND ((? = 1 AND `x` IS NULL) OR (`x` = ?)) AND ((? = 1 AND `y` IS NULL) OR (`y` = ?)))"
		...
		Private Sub InitCommandCollection()
			...
			Me._commandCollection(0).CommandText = "SELECT id, alliance, player, x, y FROM alt"
		...
		Public Overridable Function Fill(dataTable As dbDataSet.altDataTable) As Integer
		...
		Public Overridable Function GetData() As dbDataSet.altDataTable
			Me.Adapter.SelectCommand = Me.CommandCollection(0)
			Dim dataTable As dbDataSet.altDataTable = New dbDataSet.altDataTable()
			Me.Adapter.Fill(dataTable)
			Return dataTable
		End Function

		<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), HelpKeyword("vs.data.TableAdapter"), DebuggerNonUserCode()>
		Public Overridable Function Update(dataTable As dbDataSet.altDataTable) As Integer
			Return Me.Adapter.Update(dataTable)
		End Function

		<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), HelpKeyword("vs.data.TableAdapter"), DebuggerNonUserCode()>
		Public Overridable Function Update(dataSet As dbDataSet) As Integer
			Return Me.Adapter.Update(dataSet, "alt")
		End Function
